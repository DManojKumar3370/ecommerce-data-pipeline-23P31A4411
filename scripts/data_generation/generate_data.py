...
def generate_customers(num_customers: int) -> pd.DataFrame:
    ...
    # Handle edge case: zero customers
    if num_customers == 0:
    ...
    df_customers["name"] = (
        df_customers["first_name"].astype(str).str.strip()
        + " "
        + df_customers["last_name"].astype(str).str.strip()
    )

    df_customers["address"] = (
        df_customers["city"].fillna("Unknown City")
        + ", "
        + df_customers["state"].fillna("Unknown State")
        + ", "
        + df_customers["country"].fillna("Unknown Country")
    )

    if df_customers["email"].nunique() < len(df_customers):
        seen = set()
        new_emails = []
        for e in df_customers["email"]:
            if e in seen:
                new_emails.append(fake.unique.email())
            else:
                new_emails.append(e)
                seen.add(e)
        df_customers["email"] = new_emails

    assert len(df_customers) == num_customers

    mandatory_fields = ["customer_id", "email", "phone", "name", "address"]
    for col in mandatory_fields:
        df_customers[col] = df_customers[col].fillna(f"missing_{col}")
...
def generate_products(num_products: int) -> pd.DataFrame:
    ...
    # Top up if we somehow generated fewer than num_products
    while len(df_products) < num_products:
        missing = num_products - len(df_products)
        cat_info = CATEGORIES["Electronics"]
        start_idx = len(df_products)
        extra = []
        for i in range(missing):
            product_id = f"PROD{start_idx + i + 1:04d}"
            subcategory = random.choice(cat_info["subcats"])
            price = round(random.uniform(cat_info["price_range"][0], cat_info["price_range"][1]), 2)
            cost = round(price * random.uniform(0.4, 0.7), 2)
            extra.append(
        ...
        df_products = pd.concat([df_products, pd.DataFrame(extra)], ignore_index=True)

    # Trim if above num_products
    df_products = df_products.head(num_products)
